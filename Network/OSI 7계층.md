...
![image](https://github.com/lielocks/WIL/assets/107406265/d43dc08d-9dc0-4bfa-9724-c3ea20da173c)
...
데이터 링크 계층의 가장 잘 알려진 예는 Ethernet 이다.

이 외에도 `HDLC 나 ADCCP` 같은 **포인트 투 포인트(point-to-point) 프로토콜** 이나 **패킷 스위칭 네트워크** 나 `LLC, ALOHA` 같은 **근거리 네트워크용 프로토콜** 이 있다.
...
이 계층은 **경로를 선택하고 주소를 정하고 경로에 따라 패킷을 전달해주는 것** 이 이 계층의 역할이다. 

이 계층의 대표적인 장비는 **`라우터`** 이며, 요즘은 *2계층의 장비 중 스위치라는 장비에 라우팅 기능을 장착한* `Layer 3 스위치` 도 있다.
...
네트워크의 주소(IP 주소)를 정의하고, **IP 패킷의 전달 및 Routing 을 담당하는 계층**
...
IP 계층에서 **IP 패킷의 `라우팅 대상` 이 됨** (Routing)
...
같은 전송계층의 UDP 가 비연결성(connectionless)인 것과는 달리, TCP는 연결지향적 이다
...
그러므로 Session 계층은 4계층과 무관하게 응용 프로그램 관점에서 봐야 한다. 
...
이 계층은 TCP / IP session 을 만들고 없애는 책임을 진다.
...
*데이터 표현이 상이한 응용 프로세스* 의 독립성을 제공하고, 암호화 한다.
