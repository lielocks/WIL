...
그렇기 때문에 *동적인 페이지* 를 제공하기 위해서 **Web Server 는 다른 곳에 도움을 요청하여 동적인 페이지를 작성해햐 합니다.**

동적인 페이지로는 임의의 이미지만을 보여주는 페이지와 같이 ***사용자가 요청한 시점에 페이지를 생성해서 전달해주는 것*** 을 의미합니다.

여기서 **Web Server 가 동적인 페이지를 제공할 수 있도록 도와주는 application** 이 **`Servlet`** 이며, **동적인 페이지를 생성하는 application** 이 **`CGI`** 입니다.
