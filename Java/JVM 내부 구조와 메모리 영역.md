JVM(자바 가상 머신)은 자바 언어에서만 사용하는 것이 아니다. 
...
위의 Interpreter 의 단점을 보완하기 위해 도입된 방식으로 반복되는 코드를 발견하여 **바이트 코드 전체를 컴파일하여 Native Code 로 변경** 하고 이후에는 해당 
...
> - 각 클래스 / 인터페이스 마다 별도의 constant poll 테이블이 존재하는데, 클래스 생성할때 참조해야할 정보들을 상수로 가지고 있는 영역이다.
>
...
new 에 의해 생성된 **클래스는 Heap Area 에 저장** 되고, **Stack Area 에는** 생성도니 클래스의 참조인 **p만 저장** 된다.
